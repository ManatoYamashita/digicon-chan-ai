...
- `.row`(33%) と `.row2`(20%) は `overflow: hidden` を持つため flex の automatic minimum size
  ...
  **H ≤ 438 の間は 0px** で、その上も細い切れ端が続く（H=480 で 26 / 16px、553 で 72 / 43px、
  ...
  24rem の下なので 0px は観測されない）。**#74 で幅>768 の `.row3` の下の余白を 2rem 足したので、
  幅>768 の行は約 20px 低くなり、0px になる高さは 387px へ上がった**（H=481 で 58.4px）。**#60 の Issue が書いた崖 454 / 384 は計算値で、
...
- **はみ出しの最下端は `.hello .desu` の bottom で決まり、`.desu` の font-size が
  ...
  | >768 | 2rem | 344.2 | **344px**（#74 以降は余白 +32px で約 376px。計算値） |
...
幅>768 のデスクトップ配置では、`.row` / `.row2` が縮む高さ（約 800px 以下）で `.row3` の下端が常に
`H − 1rem` に来る。見出し `.hello .title`（`5rem` 固定）の行の下端は `H − 48`、ナビ（`.pillBar`、
`bottom: 24px` + 高さ 44px）の上端は `H − 68` になり、**高さに関係なく 20px（インクでは 17px）潜っていた。**
ナビの左端は `50vw` なので、幅が狭いほど「ちゃん」がナビの下に入る。

- **`@media (min-width: 769px)` で `.row3` の `margin-bottom` を `3rem` にした。** 24rem のコンパクト表示では
  `1rem` に戻す（見出しが `clamp(1.5rem, 8vh, 3rem)` まで縮み、横に重ならないうえ、上寄せなので余白が溢れに直結する）
- **持ち上げるのは見出しだけでよい。** `.desu` は x ≈ 96〜266px で、ナビの左端（最小 384.5px）より左にある。
  Issue の候補「ナビの高さぶん 68px 空ける」だと、溢れの崖（344px + 余白）が 24rem（384px）を越える
- 採らなかった候補: 見出しを `vh` で縮める（縮む帯では重なりが高さに依らず 20px 一定なので比例は合わない）、
  ナビを見出しの右へ収める（844 幅で残り約 148px しかなく、ラベルが入らない）
- 結果（本番ビルド、`reduce`、n=1）: 844×390〜1280×800 の 13 サイズ（タッチ 4 つを含む）で、見出しのインクと
  ナビの重なりが **0**（修正前は 1280×800 を除く 12 サイズで 814〜4,846px²）。インクの下端はナビの上端より 14.8px 上。既定の文字サイズを
  20px にしても 0。幅≤768 の 4 サイズ（640×400 / 390×844 / 750×326 / 320×256）と 1280×300 は幾何が修正前と一致。Tab 一巡は 14 / 11 停止で不変
- **重なりは `Range.getClientRects()` ではなくインクで測る。** 文字の矩形は content area（`line-height: 1` の
  行より 18px 下まで）を返すので、修正後も 0.5〜2.9% の「重なり」が残って見える。インクの下端は、1 文字ずつの
  Range の `top` + canvas の `measureText` の `fontBoundingBoxAscent` + `actualBoundingBoxDescent` で出す
  （`/` の見出しは `reduce` では SplitText が分割しないので、テキストノードを 1 文字ずつ Range で切る）
- **640×400 の「ですっ！」がナビに食い込む件は別の現象で、#80 で直した**（下記）

### 幅≤768 の 385〜416px でリンクのバーと「ですっ！」がナビに潜る（#80）

幅≤768 で行が 0px に潰れても、行の上の margin と gap（`.row` 1rem + gap 1rem + `.row2` 2rem + gap 1rem = 80px）は残る。
`.row3` は `.navbar` の min-content 17rem で縮まないので、H ≤ 438px では padding-bottom 70px の内側からはみ出し、
下端が 352px に固定される。ナビの上端は `H − 56` なので、**H ≤ 410〜411px で `.desu` と `.sidebar` の下端がナビの下に入っていた**
（本番 `cff27e9` で幅 320〜768 × H 385〜440 を 1px 走査。n=1）。24rem 以下はコンパクト表示が行ごと落とすので、その上の帯だけが残っていた。

- `@media (max-width: 768px) and (max-height: 26rem)` で `.back` の `gap` を 0 にし、`.row` の `margin-top` を 0、
  `.row2` の `margin-top` を `3rem` にした。`.row3` は 32px 上がり、`.sounds` は 16px だけ上がる
- **`.row2` より上を詰めると `.sounds` も同じだけ上へ動く。** 最初の案（`.row` と `.row2` の margin-top を両方 0 にして 48px 回収）は、
  320×385 で `.sounds` の上端が -10px になり画面外へ出た。`.row2` と `.row3` の間の gap だけは `.sounds` を動かさずに回収できる
  （gap を 0 にし、`.row` と `.row2` の間のぶんを `.row2` の margin へ移す）
- 閾値 26rem は既定で 416px。20px の文字サイズでは 520px で、どちらも行が 0px の帯（23rem + 70px 以下）の内側に収まる
- 結果（本番ビルド、`reduce`、n=1）: 幅 320 / 390 / 500 / 640 / 768 × H 385〜440 の 1px 走査で、ナビとの重なりは 0、溢れは 0、
  `.sounds` の上端は 22px 以上で 3 ボタンとも `elementFromPoint` で到達可。20px の文字サイズで 640×481〜521 も重なり 0。
  帯の外の 17 サイズは幾何が修正前と一致し、Tab 一巡も 14 / 11 停止で同じ
- `.sounds` と挨拶の吹き出しの重なりは面積が増える（390×385 で 3,207 → 6,349px²）。ただし 390×400 で「Hello」の吹き出しが
  `.sounds`（`z-index: 2`）の裏に隠れるのは修正前も同じで、新しく見えなくなるものは無い
...
- 圧縮後の自然高さは Music・Card とも **64px**。行が 64px に達するのは幅≤768 で H ≈ 541px なので、
  ...
  - #74 以降の幅>768 は、481px の行が **58.4px** になる。それでも圧縮後の中身は欠けない
    （幅 769〜1600 の 5 幅 × H=476〜496 の 1px 走査で、ジャケット・Play・シークバー・時間・Card の画像・
    タイトル・サムネイルの可視率がすべて 1.0。シークバーの下端 60.8px は player の下端 74.4px の内側）。
    **「64px」は幅≤768 の値で、幅>768 では下限にならない。** 閾値は 30rem のまま
...
- **`/about` のリンクカードの下地は紺の半透明（`$link-bg`）。白の半透明に戻さない**（#77）
  - 白 15% だと、下の青と混ざって白い文字が最小 3.2:1 にしかならない。`rgba(0,30,80,.35)` で最小 6.5:1、
    hover の `.45` で 7.5:1（本番ビルド、Chrome headless、`reduce`、1280×800 / 390×844、n=1）
  - 採らなかった案: 文字を `#0b2545` にする（最小 4.52:1。余裕が 0.02 しかない）、文字を 18.67px / 700 にして
    大きな文字の 3:1 を使う（3.22:1。値は改善せず基準を変えるだけ）
  - 測り方: 文字を `color: transparent` にして撮った背景の画素と文字色の比を、**テキストノードの矩形だけ**で取る。
    `selectNodeContents(カード)` だと白いアイコンの矩形まで入り、最小が 1:1 になる
- `components/menu.tsx` のナビはピルバーだけ。以前あった FAB（`.fab`・`.trigger`・`.menu`・`.overlay`）は
  `display: none !important` で一度も描かれていなかったので消した。`/about` の `.glassCard` も参照が無く消した
